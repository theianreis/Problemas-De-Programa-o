...
double calcular_sin(double x, int n) {
    if (n == 0) {
        return x; 
    } else {
        int expoente = 2 * n + 1; 
        ...
        if (n % 2 == 0) {
            return termo + calcular_sin(x, n - 1); 
        } else {
            return -termo + calcular_sin(x, n - 1); 
        }
    }
...
int main() {
    ...
    double resultado = calcular_sin(x, n - 1); 
    printf("O valor aproximado de sin(%.2lf) com %d termos é: %.6lf\n", x, n, resultado);
